...
export default function StudentChatbot({ id }: StudentChatbotProps) {
  ...
  useEffect(() => {
    const getChatbotDetails = async () => {
      // const storedData = sessionStorage.getItem("chatbotDetails")
      try {
        // if (storedData) {
        //   setChatbotDetails(JSON.parse(storedData))
    ...
    <div className="flex h-screen w-full items-center justify-center space-x-8">
      ...
      <div className="w-[25%]">
        <div className="grid grid-cols-5 space-x-4">
          <h1 className="col-span-4 mb-2 rounded-2xl bg-zinc-100 p-2 text-center font-semibold">
            {student?.name}
          </h1>
          <Button className="col-span-1" size="icon" onClick={() => signOut()}>
            <LogOut className="h-5 w-5" />
          </Button>
        </div>
        <Card className="w-full rounded-xl bg-zinc-100 shadow-xl">
          <CardHeader>
            <CardTitle>End Conversation</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="h-[430px] w-full overflow-y-auto">
              ...
              <div className="mt-8 h-[230px] w-full rounded-xl bg-zinc-200 p-4 drop-shadow-md">
                Hello
              </div>
            </div>
          </CardContent>
