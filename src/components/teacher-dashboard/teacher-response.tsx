...
interface TeacherResponseViewProps {
  responseid: string
}

export default function TeacherResponseView({
  responseid,
}: TeacherResponseViewProps) {
  ...
  useEffect(() => {
    const getResponseDetails = async () => {
      ...
      try {
        if (storedData) {
          setResponseDetails(JSON.parse(storedData))
  ...
  return (
    <div className="flex w-full flex-col items-center justify-center">
      <h1 className="pt-4 text-center text-lg font-medium">
        Response sheet of{" "}
        <strong>
          {responseDetails?.student?.name
            ? responseDetails?.student?.name
            : "Student"}
        </strong>
      </h1>
      <Card className="my-4 w-[85%] rounded-xl shadow-xl">
        <CardHeader>
